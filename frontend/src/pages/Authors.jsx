...
import { Plus, Edit2, Trash2, X, Users, MessageSquare } from 'lucide-react';
...
const Authors = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/authors/${currentAuthorId}`, formData);
        setSuccess('Author details updated.');
      } else {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Error occurred while saving author.');
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this author? This might affect catalog references.')) {
      try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Authors Directory</h1>
          <p className="text-slate-500 text-sm mt-0.5">Manage authors contributing to the library catalog.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {authors.length === 0 ? (
          <p className="text-sm text-slate-400 py-8 text-center col-span-full">No authors recorded yet.</p>
        ) : (
          authors.map((author) => (
            <div key={author.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
              <div>
                <h3 className="font-bold text-slate-800 text-base">{author.name}</h3>
                <p className="text-slate-500 text-xs mt-2 line-clamp-3 leading-relaxed">
                  {author.bio || <span className="italic text-slate-350">No biography details provided.</span>}
                </p>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900 bg-opacity-40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-slate-100 relative">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
              ...
              {isEditing ? 'Edit Author Profile' : 'Add New Author'}
            </h3>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5 flex items-center gap-1">
                  <MessageSquare className="h-3.5 w-3.5" /> Biography / Summary
                </label>
                <textarea
                  value={formData.bio}
                  onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
                  className="w-full rounded-lg border border-slate-200 py-2.5 px-3.5 text-sm text-slate-700 placeholder-slate-400 outline-none focus:border-indigo-500 transition-colors h-28 resize-none"
                  placeholder="Author description and details..."
                />
              </div>
